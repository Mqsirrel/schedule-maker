...
export class ScheduleMakerApp {
  ...
  _init() {
    ...
    this.scheduleController = new ScheduleController({
      state: this.state,
      getFilterState: () => this.filterBar.getFilterState()
    });
  ...
  async generateSchedules() {
    ...
    const result = await this.scheduleController.generate(this.wantedCourseGroups);
    const schedules = result.schedules;
    if (!schedules.length) {
      this.notification.showError(t('toast_no_schedules_found'));
      this._renderConflictDiagnostics(this.scheduleController.diagnoseConflicts(this.wantedCourseGroups));
  ...
  applyFilters(filters) {
    const result = this.scheduleController.filter(filters);
    this._updateDisplayState();
  ...
  _renderConflictDiagnostics(bottlenecks) {
    ...
    const items = bottlenecks.map(b => `<li>${t('conflict_between_courses', { c1: `<strong>${this._escapeHtml(b.courseA)}</strong>`, c2: `<strong>${this._escapeHtml(b.courseB)}</strong>` })}</li>`).join('');
  ...
  renderCurrentSchedule() {
    ...
    if (!current) return;
    this.currScheduleIndex.textContent = this.currentScheduleIndex + 1;
    this.totalSchedulesCount.textContent = this.filteredSchedules.length;
